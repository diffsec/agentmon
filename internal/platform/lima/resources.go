...
func (r *ResourceLimiter) ensureAgentmonCgroup() {
	...
	_, _ = r.platform.RunInLima("sudo", "sh", "-c",
...
func (r *ResourceLimiter) Apply(config platform.ResourceConfig) (platform.ResourceHandle, error) {
	...
	if config.MaxMemoryMB > 0 {
		...
		if _, err := r.platform.RunInLima("sudo", "sh", "-c",
	...
	if config.MaxDiskReadMBps > 0 || config.MaxDiskWriteMBps > 0 {
		...
		deviceOut, err := r.platform.RunInLima("sh", "-c",
...
func (h *ResourceHandle) AssignProcess(pid int) error {
	...
	_, err := h.platform.RunInLima("sudo", "sh", "-c",
...
func (h *ResourceHandle) Stats() platform.ResourceStats {
	...
	// Read memory.current
	if out, err := h.platform.RunInLima("cat", h.cgPath+"/memory.current"); err == nil {
	...
	if out, err := h.platform.RunInLima("cat", h.cgPath+"/cpu.stat"); err == nil {
	...
	if out, err := h.platform.RunInLima("cat", h.cgPath+"/cgroup.procs"); err == nil {
	...
	if out, err := h.platform.RunInLima("cat", h.cgPath+"/io.stat"); err == nil {
		for _, line := range strings.Split(out, "\n") {
