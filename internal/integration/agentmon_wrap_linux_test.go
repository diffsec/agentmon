...
func TestWrapStrongMode_SetsInSessionMarker(t *testing.T) {
	...
	exitCode, outputReader, err := ctr.Exec(ctx, []string{
		"/bin/sh", "-lc",
		`timeout 20s env AGENTMON_NO_AUTO=1 AGENTMON_API_KEY=test-key /usr/local/bin/agentmon --server http://127.0.0.1:18080 wrap -- /bin/sh -c 'if [ -n "$AGENTMON_IN_SESSION" ]; then echo MARKER_SET; else echo MARKER_UNSET; fi' 2>&1`,
	})
	if err != nil {
