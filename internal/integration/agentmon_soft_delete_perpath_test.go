...
// TestAgentMon_SoftDelete_PerPathPolicy is the integration-level regression test
...
func TestAgentMon_SoftDelete_PerPathPolicy(t *testing.T) {
	...
	// Restore via agentmon built-in command.
	restoreCmd := types.ExecRequest{
		Command: "agentmon",
		Args:    []string{"trash", "restore", token},
		Env: map[string]string{
			"AGENTMON_SESSION_ID": sess.ID,
		},
	}
...
sandbox:
  fuse:
    ...
    audit:
      enabled: true
      mode: "monitor"
      trash_path: "/workspace/.agentmon_trash"
  network:
    enabled: false
  unix_sockets:
    enabled: false
