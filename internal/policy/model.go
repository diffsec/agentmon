...
type Policy struct {
	...
	// MCPInspectRules select which MCP tool calls have their arguments
	// inspected. They do not authorise a tool call — see MCPInspectRule.
	MCPInspectRules []MCPInspectRule `yaml:"mcp_inspect_rules,omitempty"`
...
// MCPInspectRule selects MCP tool calls whose arguments are inspected.
//
// It is deliberately NOT an authorisation mechanism, and `decision` accepts
// only `inspect`. Whether a tool may be called at all is decided by
// mcp_rules / sandbox.mcp, evaluated by mcpinspect.PolicyEvaluator, which
// already fails closed under `tool_policy: allowlist`. Every other rule kind
// in this schema falls through to a default deny; this one falls through to
// no inspection, because a second default deny here would block every tool
// call in every existing deployment the moment an operator added one
// inspection rule. Refusing `decision: deny` is what keeps that default from
// being read as an allowlist that leaks.
type MCPInspectRule struct {
	Name        string `yaml:"name"`
	Description string `yaml:"description,omitempty"`

	// Servers matches the MCP server ID, as glob patterns. Empty matches
	// every server.
	Servers []string `yaml:"servers,omitempty"`
	// Tools matches the tool name, as glob patterns. Empty matches every
	// tool.
	Tools []string `yaml:"tools,omitempty"`

	// Decision must be "inspect".
	Decision string `yaml:"decision"`
	Message  string `yaml:"message,omitempty"`
