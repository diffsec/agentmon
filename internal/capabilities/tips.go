...
var tipsByBackend = map[string][]reasonTip{
	...
	// The blocker is code, not paperwork: the entitlement is grantable and the
	// provider classes exist, but the system extension never enters
	// NetworkExtension mode, so there is nothing for an operator to enable.
	"network-extension": {{Tip: Tip{Feature: "network-extension", Impact: "Network and DNS rules are not enforced on macOS", Action: "Not yet implemented: the system extension does not start its NetworkExtension providers"}}},
	// Windows
...
func GenerateTipsFromDomains(domains []ProtectionDomain) []Tip {
	// One backend can protect several domains -- esf covers both File
	// Protection and Command Control -- so emit one tip per backend and sum the
	// weight it would recover. Emitting per (domain, backend) duplicated the
	// tip and understated the payoff of fixing it.
	var order []string
	weight := make(map[string]int)
	base := make(map[string]Tip)
	...
	for _, d := range domains {
		...
		for _, b := range d.Backends {
			...
			if _, seen := weight[b.Name]; !seen {
				order = append(order, b.Name)
				base[b.Name] = *tip
			}
			weight[b.Name] += d.Weight
		}
	}

	tips := make([]Tip, 0, len(order))
	for _, name := range order {
		t := base[name]
		t.Impact = fmt.Sprintf("%s (+%d pts)", t.Impact, weight[name])
		tips = append(tips, t)
	}
	return tips
}
