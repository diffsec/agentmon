...
- **ESF+NE (Alpha):** Build and notarize from source (`make build-macos-enterprise`), then `agentmon activate-extension`. There is no published release or Homebrew tap yet.
...
- Lima is NOT auto-detected. It must be requested via config: `platform.mode: darwin-lima`.
  Auto-detection was removed because it fired whenever any Lima VM was running,
  so a developer with Colima up for Docker silently got a different enforcement
  backend than the one they thought they were testing.
