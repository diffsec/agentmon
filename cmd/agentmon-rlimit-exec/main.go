...
// agentmon-rlimit-exec is a wrapper that applies resource limits before exec'ing a command.
//
// Usage:
//
//	AGENTMON_RLIMIT_AS=<bytes> agentmon-rlimit-exec <command> [args...]
//
...
func main() {
	...
	if limitStr := os.Getenv("AGENTMON_RLIMIT_AS"); limitStr != "" {
		...
		if err := unix.Getrlimit(unix.RLIMIT_AS, &current); err != nil {
			fmt.Fprintf(os.Stderr, "agentmon-rlimit-exec: getrlimit failed: %v\n", err)
			os.Exit(1)
		}
