...
const ptracerLibName = "libagentmon-ptracer.so"
...
//  2. /usr/lib/agentmon/ (deb/rpm install path)
func setupPtracerPreload(serverPID int, yamaActive bool) {
	...
	os.Setenv("AGENTMON_SERVER_PID", strconv.Itoa(serverPID))
...
func findPtracerLib() string {
	...
	candidate := filepath.Join("/usr/lib/agentmon", ptracerLibName)
	if _, err := os.Stat(candidate); err == nil {
		return candidate
	}
