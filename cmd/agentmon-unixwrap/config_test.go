...
func TestLoadConfig(t *testing.T) {
	...
	t.Run("parses config from env", func(t *testing.T) {
		t.Setenv("AGENTMON_SECCOMP_CONFIG", `{"unix_socket_enabled":false,"blocked_syscalls":["ptrace","mount"]}`)
		cfg, err := loadConfig()
		require.NoError(t, err)
	...
	t.Run("error on invalid json", func(t *testing.T) {
		t.Setenv("AGENTMON_SECCOMP_CONFIG", `{invalid json}`)
		_, err := loadConfig()
		require.Error(t, err)
	})
}
...
func TestLoadConfig_WithExecve(t *testing.T) {
	t.Setenv("AGENTMON_SECCOMP_CONFIG", `{"unix_socket_enabled":true,"execve_enabled":true}`)
