...
// (diffsec/agentmon#271): the inherited PATH is empty, exec.LookPath
...
// so OC-style failures can be diagnosed from the agentmon-server logs
...
func TestResolveCommandPath_FallbackSkipsNonExecutableCandidate(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(dir1, "agentmon-271-probe"), []byte("notexec"), 0o644))
	// dir2 has the executable.
	exe := filepath.Join(dir2, "agentmon-271-probe")
	...
	t.Setenv("PATH", "")
	resolved, err := resolveCommandPath("agentmon-271-probe")
	require.NoError(t, err)
