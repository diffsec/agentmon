...
// agentmon-unixwrap: installs seccomp user-notify for AF_UNIX sockets, sends notify fd
...
func main() {
	...
	args := applyArgv0Override(os.Args[2:], os.Getenv("AGENTMON_UNIXWRAP_ARGV0"))
...
// trimming whitespace). The shim sets AGENTMON_UNIXWRAP_ARGV0 so
...
func applyLandlock(cfg *WrapperConfig) error {
	...
	// Allow network by default — agentmon proxy handles network policy.
