/*
 * libagentmon-ptracer.so — LD_PRELOAD library for Yama ptrace_scope workaround.
 *
 * Under Yama ptrace_scope=1 (Ubuntu/Debian default), only ancestor processes
 * can use ProcessVMReadv on a target. In the agentmon wrap path, the server is
 ...
 *
 * The server PID is read from the AGENTMON_SERVER_PID environment variable.
 *
 * Build: gcc -shared -fPIC -Os -o libagentmon-ptracer.so ptracer.c
 */
...
static void agentmon_set_ptracer(void) {
    const char *s = getenv("AGENTMON_SERVER_PID");
    if (s) {
