...
func main() {
	...
	if confErr != nil {
		if strings.HasPrefix(confErr.Error(), "shim.conf:") {
			fatalWithHint(127,
				fmt.Sprintf("agentmon-shell-shim: %v", confErr),
	...
	// the agentmon CLI bypass below.  We capture the error rather than fataling
	...
	// Agentmon CLI bypass: if the command being run IS the agentmon binary,
	// exec the real shell directly. The agentmon CLI connects back to the
	// server, which would deadlock if the server is handling this shim's
	// exec request. This applies to: agentmon detect, agentmon --version,
	// agentmon debug policy-test, agentmon trash list, etc.
	// Checked early so the kernelinstall branch below can also skip for
	// agentmon-CLI invocations without duplicating the check.
	if isAgentmonCommand(os.Args[1:]) {
		if realShellErr != nil {
			fatalWithHint(127,
				fmt.Sprintf("agentmon-shell-shim: resolve real shell: %v", realShellErr),
	...
	// needs them (kernel-install wrap-init, or the final agentmon exec invocation).
	// Bypass paths (in-session, agentmon-CLI, non-interactive) do NOT call
	...
	resolveSession := func() {
		...
		if sessErr != nil {
			fatalWithHint(127,
				fmt.Sprintf("agentmon-shell-shim: resolve session id: %v", sessErr),
				"Set AGENTMON_SESSION_ID (best), or set AGENTMON_SESSION_FILE to a writable file path for a stable ID.",
			)
		}
	...
	// AGENTMON_IN_SESSION=1 recursion guard because that env var is
	...
	// command to wrap), for agentmon-CLI invocations (already handled above),
	...
	if len(os.Args) > 1 && realShellErr == nil {
		...
		if modeErr != nil {
			fatalWithHint(126, "agentmon-shell-shim: shim_install mode: "+modeErr.Error(),
				"Set shim_install in /etc/agentmon/shim.conf or AGENTMON_SHIM_INSTALL to one of: auto, on, off.")
		}
		if mode != kernelinstall.ModeOff {
			...
			if installErr != nil {
				fatalWithHint(126, "agentmon-shell-shim: kernel install: "+installErr.Error(),
	...
	// Recursion guard: when agentmon executes a process, it sets AGENTMON_IN_SESSION=1.
	...
	inSession := strings.TrimSpace(os.Getenv("AGENTMON_IN_SESSION"))
	debugLog("recursion check: AGENTMON_IN_SESSION=%q", inSession)
	if inSession == "1" {
		...
		if inSessErr != nil {
			...
			if inSessErr != nil {
				fatalWithHint(127,
					fmt.Sprintf("agentmon-shell-shim: in-session: could not find %s", shellName),
	...
	// Non-bypass paths (non-interactive, ready-gate, and the full agentmon exec
	...
	// agentmon sessions is handled by AGENTMON_IN_SESSION (checked above).
	//
	// AGENTMON_SHIM_FORCE=1 overrides this bypass for environments like sandbox
	// platforms where commands are always non-interactive but still require
	// policy enforcement (e.g. Blaxel, E2B sandbox APIs).
	//
	// /etc/agentmon/shim.conf with force=true also overrides the bypass, for
	// platforms where env vars cannot be injected (e.g. exe.dev).
	// Precedence: AGENTMON_SHIM_FORCE=1 (env) > config file > default (false).
	// Note: env can only ADD enforcement, never remove it.
	forceShim := strings.TrimSpace(os.Getenv("AGENTMON_SHIM_FORCE"))
	forceFromEnv := forceShim == "1"
	switch {
	case forceFromEnv:
		debugLog("AGENTMON_SHIM_FORCE=1: enforcing policy despite non-interactive stdin")
	case conf.Force:
		forceShim = "1"
	...
	// agentmon server is listening before attempting enforcement. If the local
	...
	// image and the shim is root's login shell — agentmon.service may not be
	...
	//
	// When AGENTMON_SHIM_FORCE=1 is set via env (not config file), the gate
	...
	if conf.ReadyGate && !forceFromEnv {
		...
		if srvErr != nil {
			...
			hint := "Fix the AGENTMON_SERVER value or unset it to use the default (http://127.0.0.1:18080)."
			if strings.ToLower(strings.TrimSpace(os.Getenv("AGENTMON_TRANSPORT"))) == "grpc" {
				hint = "Fix the AGENTMON_GRPC_ADDR value or unset it to use the default (127.0.0.1:9090)."
			}
			fatalWithHint(127,
				fmt.Sprintf("agentmon-shell-shim: ready_gate: %v", srvErr),
				hint,
			)
		}
		...
		if !reachable {
			...
			if local {
				...
				fatalWithHint(127,
					fmt.Sprintf("agentmon-shell-shim: local server dial error at %s: %v", srvAddr, dialErr),
					"Check unix socket permissions or server configuration.",
				)
			}
			fatalWithHint(127,
				fmt.Sprintf("agentmon-shell-shim: remote server not reachable at %s", srvAddr),
				"The agentmon server is configured as a remote endpoint and is not responding. Check server availability.",
			)
		}
	}

	agentmonBin, err := resolveAgentmonBin()
	if err != nil {
		hint := "Set AGENTMON_BIN=/path/to/agentmon or ensure `agentmon` is available on PATH."
		if v := strings.TrimSpace(os.Getenv("AGENTMON_BIN")); v != "" {
			hint = fmt.Sprintf("AGENTMON_BIN is set to %q but wasn't executable; fix it or unset it to use PATH.", v)
		}
		fatalWithHint(127, fmt.Sprintf("agentmon-shell-shim: resolve agentmon: %v", err), hint)
	}

	// Resolve the session ID now if the kernelinstall branch did not already
	// do so (mode was off, or realShell was unresolved).  The agentmon exec
	...
	// to -c so the command goes through agentmon policy enforcement.
	shellArgs := os.Args[1:]
	...
	// Run agentmon exec as a child process rather than replacing the shim via
	...
	runAndExit(agentmonBin, "", args[1:], os.Environ())
}
...
// isAgentmonCommand checks if the command being executed is the agentmon binary.
// This prevents a deadlock where the shim routes agentmon CLI commands through
...
func isAgentmonCommand(args []string) bool {
	...
	// bypass for chained commands like "agentmon detect; rm -rf /".
	...
	// - "exec agentmon detect" → "agentmon"
	// - "env FOO=1 agentmon detect" → "agentmon"
	// - "env -i agentmon detect" → "agentmon"
...
func resolveAgentmonBin() (string, error) {
	if v := strings.TrimSpace(os.Getenv("AGENTMON_BIN")); v != "" {
		return exec.LookPath(v)
	}
	return exec.LookPath("agentmon")
}
...
func execOrExit(path string, argv []string, env []string) {
	if err := syscall.Exec(path, argv, env); err != nil {
		fatalWithHint(127,
			fmt.Sprintf("agentmon-shell-shim: exec %s: %v", path, err),
			"If you see 'permission denied' in a container, check that the shim and agentmon binaries are executable.",
		)
	}
}
...
func runAndExit(path string, argv0 string, args []string, env []string) {
	...
	if err != nil {
		fatalWithHint(127, fmt.Sprintf("agentmon-shell-shim: stdout pipe: %v", err), "")
	}
	stderrPipe, err := cmd.StderrPipe()
	if err != nil {
		fatalWithHint(127, fmt.Sprintf("agentmon-shell-shim: stderr pipe: %v", err), "")
	}

	debugLog("runAndExit: path=%s argv0=%s args=%v", path, argv0, args)
	if err := cmd.Start(); err != nil {
		fatalWithHint(127,
			fmt.Sprintf("agentmon-shell-shim: run %s: %v", path, err),
			"If you see 'permission denied' in a container, check that the shim and agentmon binaries are executable.",
		)
	}
	...
	if waitErr != nil {
		...
		fatalWithHint(127,
			fmt.Sprintf("agentmon-shell-shim: run %s: %v", path, waitErr),
			"If you see 'permission denied' in a container, check that the shim and agentmon binaries are executable.",
		)
	}
	os.Exit(0)
...
func fatalWithHint(code int, msg string, hint string) {
	...
	if strings.TrimSpace(os.Getenv("AGENTMON_SHIM_DEBUG")) == "1" {
...
func debugLog(format string, args ...any) {
	if strings.TrimSpace(os.Getenv("AGENTMON_SHIM_DEBUG")) == "1" {
		_, _ = fmt.Fprintf(os.Stderr, "agentmon-shell-shim: "+format+"\n", args...)
	}
}
...
// serverAddrFromEnv returns the network type and address of the agentmon server
// for a readiness probe. It follows the same transport selection as the agentmon
// CLI: when AGENTMON_TRANSPORT=grpc, probes AGENTMON_GRPC_ADDR instead of
// AGENTMON_SERVER. For HTTP transport (the default), reads AGENTMON_SERVER
...
func serverAddrFromEnv() (network, addr string, err error) {
	...
	transport := strings.ToLower(strings.TrimSpace(os.Getenv("AGENTMON_TRANSPORT")))
	if transport != "" && transport != "http" && transport != "grpc" {
		return "", "", fmt.Errorf("invalid AGENTMON_TRANSPORT value %q (expected http or grpc)", transport)
	}
	if transport == "grpc" {
		grpcAddr := strings.TrimSpace(os.Getenv("AGENTMON_GRPC_ADDR"))
		if grpcAddr == "" {
		...
		if _, _, splitErr := net.SplitHostPort(grpcAddr); splitErr != nil {
			return "", "", fmt.Errorf("invalid AGENTMON_GRPC_ADDR value %q: %w", grpcAddr, splitErr)
		}
		return "tcp", grpcAddr, nil
	}

	raw := strings.TrimSpace(os.Getenv("AGENTMON_SERVER"))
	if raw == "" {
		return "tcp", "127.0.0.1:18080", nil
	}
	u, parseErr := url.Parse(raw)
	if parseErr != nil {
		return "", "", fmt.Errorf("invalid AGENTMON_SERVER value %q: %w", raw, parseErr)
	}
	switch u.Scheme {
	case "unix":
		...
		if sockPath == "" {
			return "", "", fmt.Errorf("invalid AGENTMON_SERVER: unix URL with empty socket path %q", raw)
		}
		return "unix", sockPath, nil
	case "http", "https":
		host := u.Hostname()
		if host == "" {
			return "", "", fmt.Errorf("invalid AGENTMON_SERVER: %s URL with empty host %q", u.Scheme, raw)
		}
		port := u.Port()
		if port == "" {
			switch u.Scheme {
	...
	default:
		return "", "", fmt.Errorf("invalid AGENTMON_SERVER: unsupported scheme %q in %q (expected http, https, or unix)", u.Scheme, raw)
	}
}

// serverReachable does a fast dial to check if the agentmon server is
...
// server startup. If a non-agentmon service occupies the port, the
// subsequent agentmon exec call fails with a clear error.
//
