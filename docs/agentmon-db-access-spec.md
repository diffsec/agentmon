# AgentMon Database Access Control - v0.9
...
**Owner:** Canyon Road
**Scope:** AgentMon OSS, Watchtower commercial control plane
...
The other consequential edit is **R10**: §12.4's "supervisor-issued process token" prose is replaced with a concrete SessionID-keyed mechanism grounded in AgentMon's existing ptrace-attach primitive, plus a new §12.5 covering listener authentication (a gap v0.7 did not address).
...
This spec gives AgentMon database-aware runtime enforcement. AgentMon classifies each statement an agent issues into a list of effects, evaluates each effect against policy with strict multi-object semantics, and emits a normalized audit event tied to the agent session and process context.

The thesis:

> AgentMon makes database access agent-aware, policy-governed, and **unavoidable for processes inside the AgentMon-governed process tree, for declared DB services, assuming the AgentMon supervisor and proxy are not compromised.**

Unavoidability rests on AgentMon's existing primitives: process interception, network rules, file rules over Unix sockets, and DNS interception. §12 spells out the threat model in full.
...
| `client_identity` | no | Glob. Matches the AgentMon-internal client identity. Available in all TLS modes. |
...
A privacy-sensitive deployment sets `log_statements: parameters_redacted` (or `none`) and `approval_statement_preview: redacted` (or `none`). With both set to redaction-or-stronger, no unredacted statement text exists anywhere in the AgentMon path.
...
agent process (SessionID = agent)
   │  TCP connect
   ▼
[ AgentMon connect_redirect ] — rewrites to per-session Unix socket
   ▼
[ AgentMon DB proxy ] (SessionID = proxy, distinct from agent)
...
2. **Listener authentication.** Proxy `accept()`s the connection, calls `getsockopt(SO_PEERCRED)`, resolves peer pid → SessionID via the AgentMon ptrace registry, verifies the SessionID matches the configured agent SessionID. Mismatch → close silently, emit `db_listener_auth_fail`.
...
> **The DB proxy is unavoidable for processes inside the AgentMon-governed process tree, for declared `db_services`, assuming the AgentMon supervisor and proxy are not compromised.**

Three explicit scoping clauses:

1. **AgentMon-governed process tree.** A process running outside AgentMon's interception (e.g., on a different host, in a different namespace not under AgentMon supervision) is out of model.
2. **Declared `db_services`.** A database hosted at a destination the operator has not declared cannot be governed; AgentMon does not know it exists.
3. **Supervisor and proxy not compromised.** Compromise of the AgentMon supervisor process or of the DB proxy itself is out of model.

Inside this scope, the threat model and defenses below apply. Outside this scope, AgentMon makes no claim. Customer-facing material should always pair the unavoidability claim with these scoping clauses.
...
1. **TCP egress** to that destination tuple is denied for every process **except the AgentMon proxy itself**, via existing `network_rules` with process-identity match. This catches custom tunnel binaries the agent might compile.
2. **DNS resolution** of the upstream hostname is intercepted by AgentMon's DNS layer. Resolved IP set is added to the denial set; CNAMEs are followed.
...
The proxy egress exemption is keyed on **AgentMon SessionID**, the existing process-tracking primitive set at ptrace attach (see `internal/ptrace/attach.go`) and consulted on every network-rule evaluation (see `internal/policy/engine.go` `CheckNetworkCtx`). The mechanism:

1. The AgentMon supervisor launches the DB proxy as a separate process under a **distinct SessionID**, outside the agent's ptrace tree. The proxy is not a child of any agent process.
...
4. Forging a SessionID at the rule-evaluation API requires either compromising the supervisor (out of model per §12.1) or escaping the ptrace tree (which the unavoidability primitives prevent). An agent cannot grant itself the exemption by renaming a binary, launching a process named `agentmon-db-proxy`, or any other in-band trick.
...
- The proxy creates a Unix domain socket at a per-session path (e.g., `/run/agentmon/<session_id>/db-proxy.sock`). The path is placed in a directory the agent's `file_rules` permit and other tenants do not.
- On `accept()`, the proxy calls `getsockopt(SO_PEERCRED)` to obtain the connecting process's `(pid, uid, gid)`.
- The proxy resolves the peer's `pid` to a TGID, then to a SessionID, via the existing AgentMon ptrace registry. The resolved SessionID must equal the proxy's configured agent SessionID.
...
- An agent running outside AgentMon's process tree.
- Compromise of the proxy itself.
- Compromise of the AgentMon supervisor.
...
| `terminate_reissue` | Inbound TLS terminated with AgentMon CA leaf. Re-establishes upstream TLS. SCRAM-SHA-256-PLUS detected at handshake → fail-closed with structured error referencing Phase 4 broker. | Full. |
...
|-------|-------------|-------------------|------------------------------|
| `db_service` | ✓ | ✓ | ✓ |
| `client_identity` (AgentMon SessionID — see §12.4) | ✓ | ✓ | ✓ |
| destination | ✓ | ✓ | ✓ |
...
- **SessionID:** AgentMon's per-session process-tracking primitive, set at ptrace attach. Network rules and listener authentication are SessionID-keyed (§12.4, §12.5). Inherited by child processes within a session; supervisor-assigned at session creation; not forgeable from inside the agent's session.
...
- AgentMon README and policy model: `https://github.com/diffsec/agentmon`
