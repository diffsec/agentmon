# agentmon Spec Closeout Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Close the remaining gaps between the current codebase and `docs/spec.md` + `docs/plans/2025-12-16-agentmon-implementation-plan.md`, while keeping changes Linux-first and test-verified.
...
**Step 1: Write failing test**
- Set env vars like `AGENTMON_HTTP_ADDR`, `AGENTMON_GRPC_ADDR` and ensure they override YAML values.
...
- In `Load()`, after YAML unmarshal + defaults, apply overrides:
  - `AGENTMON_HTTP_ADDR` → `cfg.Server.HTTP.Addr`
  - `AGENTMON_GRPC_ADDR` → `cfg.Server.GRPC.Addr`
  - (Optional) `AGENTMON_DATA_DIR` to derive defaults for `sessions.base_dir` and `audit.storage.sqlite_path`.
...
### Task 4.1: Implement `agentmon session attach SESSION_ID`
...
- Add Cobra subcommand `session attach`:
  - REPL loop: print prompt `agentmon:<session>:<cwd>$ ` (fetch cwd via `GET /sessions/{id}`).
...
### Task 4.2: Add `agentmon exec --json '...'`
...
### Task 4.3: Add true streaming exec (`agentmon exec --stream`)
...
### Task 5.1: `agentmon policy list/show/validate`
...
- Verify `agentmon policy` subcommands exist.
...
### Task 5.2: `agentmon config show/validate`
...
**Step 1: Write failing CLI test**
- Verify `agentmon config show` exists.
...
**Files:**
- Add: `proto/agentmon/v1/agentmon.proto`
- Add: `internal/grpcserver/server.go`
- Add generated: `proto/agentmon/v1/agentmon.pb.go`, `proto/agentmon/v1/agentmon_grpc.pb.go`
...
**Edits:**
- Document `AGENTMON_NO_AUTO=1` and the auto-start/auto-create behavior of `agentmon exec`.
...
**Verify:**
- Run: `rg -n \"AGENTMON_NO_AUTO\" docs/spec.md README.md` and ensure it’s referenced.
