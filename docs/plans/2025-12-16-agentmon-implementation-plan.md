# agentmon Implementation Plan (Linux-first, approvals later)
...
Implement **agentmon** as a daemon + CLI that provides:
...
- **CLI**:
  - `agentmon events query --session <id> --type file_write --since 1h --path-like '%.env%' --decision deny --limit 200`
  - `agentmon events tail --session <id>` (follows SSE stream; optional `--also-jsonl` tail helper for local debugging)
  - Support **both** query modes:
    - Default: query via daemon API (works for remote server, auth-aware)
    - Offline: `agentmon events query --direct-db [--db-path <path>] ...` (reads SQLite locally when server isn’t running)
...
Adopt the structure in `docs/project-structure.md` (Go module, `cmd/agentmon`, `internal/*`, `pkg/*`, `api/proto/*`, etc.).
...
- `agentmon server`: long-running daemon hosting:
...
- `agentmon` CLI: talks to the daemon (HTTP first; gRPC later).
...
- `agentmon --help` works (CLI scaffolding)
...
- Query path:
  ...
  - CLI `agentmon events query` that hits the API (not direct DB access by default)
...
- Minimal gauges/counters:
  ...
  - `agentmon_events_total` + `agentmon_events_by_type_total`
