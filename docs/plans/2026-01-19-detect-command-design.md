# agentmon detect Command Design
...
**Architecture:** Two subcommands under `agentmon detect` - one for inspection, one for config generation. Cross-platform support with unified output structure containing platform-specific fields. Actionable tips guide users to enable missing capabilities.
...
```
agentmon detect [--output table|json|yaml]
agentmon detect config [--output <filepath>]
```
...
**Behavior:**
- Prints to stdout by default (pipe-friendly: `agentmon detect config > security.yaml`)
...
| `minifilter` | No kernel-level file interception | Install agentmon minifilter driver (requires Administrator). |
...
```yaml
# Generated by: agentmon detect config
# Platform: linux
...
4. **Integration test** that runs `agentmon detect` and verifies output format
