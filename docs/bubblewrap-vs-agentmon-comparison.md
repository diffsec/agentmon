# Bubblewrap vs agentmon: Linux Container Sandboxing Comparison

This document compares Bubblewrap (bwrap), a lightweight unprivileged sandboxing tool, with agentmon running in a Linux container environment for AI agent security.
...
**Bottom Line**: Bubblewrap is a flexible, low-level sandbox *construction* tool—it provides the primitives for isolation but requires manual configuration and offers no runtime policy enforcement. agentmon is a complete security *platform* with continuous policy evaluation, semantic understanding of operations, and defense-in-depth architecture.
...
### agentmon Architecture

agentmon provides multi-layer, continuous enforcement with semantic policy evaluation:

```
┌──────────────────────────────────────────────────────────────────────────────┐
│                         AGENTMON SECURITY ARCHITECTURE                          │
│                                                                                │
│  ┌──────────────────────────────────────────────────────────────────────────┐ │
...
| Feature | Bubblewrap | agentmon | Notes |
|---------|:----------:|:-------:|-------|
| **Namespace Isolation** |
...
| User-notify mode | ❌ | ✅ | agentmon uses SECCOMP_RET_USER_NOTIF |
| Dangerous syscall blocking | ❌ Manual | ✅ Default | ptrace, mount, kexec, etc. |
| Per-syscall policy | ❌ | ✅ | Runtime policy decisions |
| **Network Control** |
| Network isolation | ✅ Binary | ✅ Granular | bwrap: isolated or not; agentmon: per-connection |
...
agentmon uses Linux namespaces as one layer of a multi-layer defense:
...
**Key Difference**: agentmon uses namespaces for process isolation but adds FUSE, eBPF, and seccomp layers for semantic policy enforcement. Bubblewrap provides namespaces only—policy logic must be built externally.
...
agentmon uses dual-mode seccomp with built-in dangerous syscall blocking:
...
**Key Difference**: agentmon provides:
...
agentmon provides multi-layer network enforcement:
...
**Key Difference**: Bubblewrap provides binary network isolation. agentmon provides granular, kernel-enforced, per-connection policy with domain awareness.
...
agentmon uses FUSE for per-operation policy enforcement:
...
**Key Difference**: Bubblewrap provides static mount-level access control. agentmon provides dynamic per-operation policy with semantic understanding of file operations.
...
agentmon uses cgroups v2 for comprehensive resource limits:

```go
// internal/limits/cgroupv2_linux.go
// Per-session cgroup at /sys/fs/cgroup/agentmon/<session-id>
...
```bash
# Memory limit
echo 1073741824 > /sys/fs/cgroup/agentmon/<session>/memory.max

# CPU limit (50% of one core)
echo "50000 100000" > /sys/fs/cgroup/agentmon/<session>/cpu.max

# Process limit
echo 100 > /sys/fs/cgroup/agentmon/<session>/pids.max

# Disk I/O limit
echo "8:0 rbps=10485760 wbps=10485760" > /sys/fs/cgroup/agentmon/<session>/io.max
```

**Key Difference**: Bubblewrap provides no resource limiting. agentmon provides comprehensive cgroups v2 enforcement for CPU, memory, I/O, and process count.
...
- agentmon daemon runs unprivileged when possible
...
**Key Difference**: Bubblewrap security depends on correct configuration by the user. agentmon provides built-in security policies with sensible defaults and defense-in-depth architecture.
...
| Requirement | Bubblewrap | agentmon |
|-------------|:----------:|:-------:|
...
- Malicious agentmon binary (assumes trusted installation)
...
- You want bubblewrap's fast namespace isolation AND agentmon's policy enforcement
- agentmon can run inside a bubblewrap sandbox for additional isolation
