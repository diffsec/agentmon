# Shim Configuration File (`/etc/agentmon/shim.conf`)

## Problem

The shell shim (`agentmon-shell-shim`) bypasses policy enforcement when stdin is not a TTY and `AGENTMON_SHIM_FORCE=1` is not set. This is correct for general use (preserves binary stdin/stdout for piped data), but breaks enforcement on platforms where:
...
Add a config file that the shim reads at startup. The file is written during `agentmon shim install-shell --force` or by the operator manually. Unlike env vars or profile scripts, the shim reads the file directly — it works regardless of how the shell was spawned.
...
```
# /etc/agentmon/shim.conf
# Written by: agentmon shim install-shell --force
force=true
```

### Config path

`/etc/agentmon/shim.conf` on both Linux and macOS. `/etc` exists on macOS, the config is system-level (not per-user), and using the same path keeps the code simple. `ShimConfPath(root)` is the single place to change this if a platform-specific path is ever needed.
...
// Prepends a comment header: "# Written by: agentmon shim install-shell".
...
- Directory `/etc/agentmon/`: mode `0o755`, owned by root. Standard `/etc` subdirectory convention.
...
### Shim bypass logic (`cmd/agentmon-shell-shim/main.go`)

Precedence: env var > config file > default (false).

```go
confRoot := "/"
if v := os.Getenv("AGENTMON_SHIM_CONF_ROOT"); v != "" {
    confRoot = v
}
...
case forceShim == "0":
    if conf.Force {
        debugLog("AGENTMON_SHIM_FORCE=0: config file force overridden by env")
    }
case conf.Force:
    forceShim = "1"
...
`AGENTMON_SHIM_CONF_ROOT` is an internal testing hook (not documented for end users) that allows integration tests to point the shim at a temp directory instead of `/`.

`AGENTMON_SHIM_FORCE=0` explicitly overrides `force=true` in the config, giving operators per-process control.
...
```
agentmon shim install-shell --root / --shim /path/to/shim --bash --force --i-understand-this-modifies-the-host
```
...
3. `cmd/agentmon-shell-shim/main.go` — read config, integrate into bypass logic
4. `cmd/agentmon-shell-shim/main_test.go` — integration tests for config + env + TTY precedence
...
### Integration tests (`cmd/agentmon-shell-shim/main_test.go`)

- No config, no env, no TTY → bypass (existing, unchanged)
- No config, `AGENTMON_SHIM_FORCE=1`, no TTY → enforce (existing, unchanged)
- Config `force=true`, no env, no TTY → enforce (new)
- Config `force=true`, `AGENTMON_SHIM_FORCE=0`, no TTY → bypass (env overrides config)
