...
**Spec:** `docs/superpowers/specs/2026-04-08-agentmon-wrap-autostart-design.md`
...
1. **`agentmon wrap`** is a subcommand that launches an "AI agent" (like `claude-code` or `codex`) as a child process with all its `exec` syscalls intercepted via seccomp / Endpoint Security / a Windows driver. The intercepted commands are routed through an `agentmon server` for policy checks, approvals, and audit logging. The full implementation is in `internal/cli/wrap.go`.

2. **The `agentmon server`** is a separate process — typically the same `agentmon` binary invoked as `agentmon server --config <path>`. It listens on `http://127.0.0.1:18080` by default. `wrap` (and `exec`) talk to it via an HTTP/gRPC client (`internal/client/cli_client.go`).

3. **Auto-start already exists for `exec`.** Look at `internal/cli/auto.go` (~170 lines) — the helpers `autoDisabled`, `isConnectionError`, `shouldAutoStartServer`, `ensureServerRunning`, `waitForHealth` are all there. `agentmon exec` and `agentmon exec --pty` already use them (`exec.go:147–154`, `exec.go:225–229`, `exec_pty.go:139–142`). `wrap.go` does not. This plan adds the same wiring to `wrap.go`.
...
6. **`AGENTMON_NO_AUTO=1`** in the environment makes `autoDisabled()` return true, which short-circuits the retry. Inherited unchanged.

7. **The test seam.** `runWrap` itself is hard to unit-test end-to-end (it calls `exec.LookPath`, sets up seccomp/ES/driver interception, launches a child process, runs a ptrace handshake on Linux, and waits for the child). So we extract the auto-start-wrapped session fetch into `fetchSessionForWrap` and unit-test that helper directly. To avoid the test forking a real `agentmon server` subprocess, we add a package-level var `ensureServerRunningFn = ensureServerRunning` that the test swaps with a stub.
...
auto-start-and-retry pattern already used by agentmon exec. On a
...
ensureServerRunning) to fork a local agentmon server, then retries
...
This is a pure refactor at the call site. The behavior change (auto-start) was added in Task 3 but is dormant until the call site switches over. After this task, `agentmon wrap` actually auto-starts the server.
...
Expected: PASS. This catches any side effects in other packages — there shouldn't be any, but agentmon has many packages and a full run is required by `CLAUDE.md` before commit.
...
git commit -m "$(cat <<'EOF'
feat(cli/wrap): auto-start agentmon server when running wrap

runWrap now delegates session reuse / creation to fetchSessionForWrap,
which auto-starts a local agentmon server on connection-refused. Users
no longer need to start the server manually before running
'agentmon wrap -- <agent>' on a default loopback config.

Behaviour matches 'agentmon exec' today: only loopback hosts on the
default port (18080) are eligible for auto-start, AGENTMON_NO_AUTO=1
...
Expected: success, `./bin/agentmon` exists and is executable.

- [ ] **Step 5.2: Ensure no agentmon server is running**

Run:
```bash
pkill -f 'agentmon server' || true
...
Run:
```bash
./bin/agentmon wrap -- bash -c 'echo hello from wrapped'
```

Expected stderr lines (in order, exact wording may vary slightly):
```
agentmon: auto-starting server (config <some path>)
agentmon: session <id> created (policy: agent-default)
agentmon: agent bash started with <mechanism> interception (pid: <n>)
agentmon: session <id> complete (agent exit code: 0)
```
...
Run:
```bash
./bin/agentmon wrap -- bash -c 'echo second run'
```
...
- [ ] **Step 5.5: Verify `AGENTMON_NO_AUTO=1` disables auto-start**

Kill the running server and try again with auto-start disabled:
```bash
pkill -f 'agentmon server'
sleep 0.5
AGENTMON_NO_AUTO=1 ./bin/agentmon wrap -- bash -c 'echo this should fail' || echo "wrap exited with non-zero (expected)"
```
