...
**Goal:** Add `/etc/agentmon/shim.conf` support so the shell shim can read configuration (starting with `force=true`) without relying on environment variables.
...
| `cmd/agentmon-shell-shim/main.go` | Modify | Read config, integrate into bypass logic |
| `cmd/agentmon-shell-shim/main_test.go` | Modify | Integration tests for config + env + TTY precedence |
...
func TestShimConfPath(t *testing.T) {
	tests := []struct {
		...
		{"/rootfs", "/rootfs/etc/agentmon/shim.conf"},
		{"", "/etc/agentmon/shim.conf"},
	}
...
**Files:**
- Modify: `cmd/agentmon-shell-shim/main.go:55-73`

**Depends on:** Task 1

- [ ] **Step 1: Modify bypass logic in `main.go`**

Note: `main.go` already imports `"github.com/diffsec/agentmon/internal/shim"` (line 11). No import changes needed.
...
Note: `AGENTMON_SHIM_CONF_ROOT` is an internal testing hook — it allows integration tests to point the shim at a temp directory. It is not documented for end users.
...
git commit -m "feat: shell shim reads /etc/agentmon/shim.conf for force setting"
```
...
**Files:**
- Modify: `cmd/agentmon-shell-shim/main_test.go`
...
func TestShimConfForce_EnvZeroOverridesConfig(t *testing.T) {
	...
	// Write config with force=true, but AGENTMON_SHIM_FORCE=0 should override.
	confDir := filepath.Join(tmp, "etc", "agentmon")
	if err := os.MkdirAll(confDir, 0o755); err != nil {
		t.Fatal(err)
	}
...
Run: `go test ./cmd/agentmon-shell-shim/ -run "TestShimConfForce" -v`
Expected: PASS
...
```bash
git add cmd/agentmon-shell-shim/main_test.go
