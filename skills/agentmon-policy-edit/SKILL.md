...
description: Use when adding, removing, or updating rules in an existing AgentMon policy, modifying security permissions, HTTP service declarations, Postgres-family database rules, resource limits, or policy YAML files
---

# Edit AgentMon Policy

## Overview
Make targeted edits to existing AgentMon security policies — add, remove, or update rules and declarations. Understands first-match-wins policy categories plus the separate Postgres-family database rule semantics.
...
- NOT for creating new policies from scratch (use agentmon-policy-create)
...
5. **Validate**
   Run: `agentmon policy validate <name>`
   If validation fails, fix and re-validate.
   If `agentmon` binary is not available, warn the user to validate manually.
...
- `passthrough` DB services can use connection rules only; statement rules are unavailable because AgentMon cannot inspect SQL.
...
- `policies.db.unavoidability: enforce` should be paired with declared `db_services`; it blocks direct DB egress from governed processes and routes through AgentMon-generated DB proxy redirects.
...
Read `skills/agentmon-policy-shared/schema-reference.md` for the complete policy YAML schema before making edits. If this file is not accessible, use the existing policy file as your reference for field names and structure. For rule categories not present in the existing file, ask the user for details rather than guessing.
