# AgentMon Policy Schema Reference

This file is the authoritative YAML-author-facing schema for AgentMon policy files.
It is read at invocation time by both the `policy-create` and `policy-edit` skills
and must be self-contained — the reader will not have access to the AgentMon source code.
...
Metadata is non-enforcing. AgentMon-generated policy bundles use it to correlate generic rule names back to higher-level sources such as DB unavoidability.
...
Declared HTTP services expose selected upstream API paths to child processes through the AgentMon proxy as `/svc/<name>/...`. Use this when the policy needs method/path control, approval gates, credential substitution, or audited HTTP service traffic. Use `network_rules` instead for simple host/port access.
...
| client_identity | string | no | AgentMon client identity selector |
...
Rules that match a `passthrough` service cannot use `db_user`, `database`, or `application_name`, because AgentMon cannot inspect StartupMessage fields through passthrough TLS. `approve` is invalid for `match_kind: cancel`. `redirect` is invalid for all connection rules.
...
```yaml
env_inject:
  BASH_ENV: "/etc/agentmon/bash_restricted.sh"
...
http_services:
  - name: stripe
    ...
    secret:
      ref: keyring://agentmon/stripe_api_key
